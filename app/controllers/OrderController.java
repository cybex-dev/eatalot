        ...
        if(AccountController.isCustomer())
            if(session("orderId") != null)
                return ok(master.render("Cart",
                        cart.render(
                                CustomerOrder.findAllMealsFromOrder(Long.parseLong(session("orderId"))),
                                CustomerOrder.findOrderById(Long.parseLong(session("orderId"))))));
            else
