...
public class UserFinance extends Controller {
    ...
    public Result doAddFunds(){
        ...
        try {
            if (RedeemedVouchers.find.query().where().ilike("voucherId", userFunds.getVoucherValue()).findList().size() != 0){
            ...
            RedeemedVouchers redeemedVoucher = new RedeemedVouchers(userFunds.getVoucherValue(), userFunds.getUserId(), new Date());
            redeemedVoucher.save();
            Voucher voucher = null;
            try {
                voucher = Voucher.find.query().where().ilike("voucherCode", userFunds.getVoucherValue()).findOne();
            }
            catch (NonUniqueResultException e){
                Logger.warn("Exception: " + e.toString() + " UserFunds::validate\nCode : " + userFunds.getVoucherValue() + " has multiple occurances!");
            }
